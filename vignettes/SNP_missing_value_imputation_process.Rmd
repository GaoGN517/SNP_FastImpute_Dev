...
errors_df <- as.data.frame(cbind(ratios, errors))
times_df <- as.data.frame(cbind(ratios, time_missingRatio))
```

```{r plot missing ratio vs error rate and time, echo = T, eval = T, fig.height = 4.5, message = F, tidy = T, tidy.opts=list(width.cutoff=35)}
plot(errors_df$ratios, errors_df$errors)
plot(times_df$ratios, times_df$time_missingRatio)
```
The result shows that as missing ratio increases, the classification error rate increases, and the processing time also increases. 

```{r, include=F}
#ggplot(times_df, aes(x = ratios, y = time_missingRatio)) + geom_point(size = 3)
```
