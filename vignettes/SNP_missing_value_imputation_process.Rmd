...
for(i in 1:ratios_len){
  errors[i] <- classification_error(df, df_fills[[i]], SNP_NA_dfs[[i]]$NP_generate_positions)
}
```

```{r plot missing ratio vs error rate and time, echo = T, eval = T, fig.height = 4.5, message = F, tidy = T, tidy.opts=list(width.cutoff=35)}
ggplot(as.data.frame(cbind(ratios, errors)), aes(x = ratios, y = errors)) + geom_point(size = 3)

ggplot(as.data.frame(cbind(ratios, time_missingRatio)), aes(x = ratios, y = time_missingRatio)) + geom_point(size = 3)
```
