...
Generate a list of data with different missing ratio from 0.05 to 0.25
```{r introduceNA}
...
```{r predict for different missing ratio before and after considering correlation, cache=T, warning=F}
...
p1 <- ggplot(errors_df, aes(x = ratios, y = errors)) + geom_point() + ylim(c(0, 0.2))
p2 <- ggplot(error_corr_df, aes(x = ratios, y = error_corr)) + geom_point() + ylim(c(0, 2))
ggarrange(p1, p2, nrow = 1)
```

The result shows that considering the correlation when selecting windows size helped to reduce error rate and increase accuracy. 